...
function TweetCard({ tweet, handleDelete }) {
  ...
  const isOwner = tweet.owner._id === userId;
  ...
  function handleEdit() {
    if (!isEditing) {
      ...
      if (!editedContent) {
        ...
        axios
          .patch(
            `${import.meta.env.VITE_REACT_APP_API_BASE}/tweets/${tweet._id}`,
  ...
  function handleLike() {
    axios
      .post(
        `${import.meta.env.VITE_REACT_APP_API_BASE}/likes/toggle/tweet/${
          tweet._id
        }`,
        {},
        {
  ...
  return (
    <div className="bg-gray-800 border border-gray-700 rounded-lg p-4 mb-4 shadow-lg">
      ...
      {isEditing ? (
        ...
        <div className="mb-4 text-gray-200">{editedContent}</div>
      )}
